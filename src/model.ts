...
function goal(v: VenueInfo): string {
  ...
  const tick = v.blockMs >= 1000 ? `${v.blockMs / 1000} s` : `${v.blockMs} ms`;
  const horizonS = Math.round((config.horizonBlocks * v.blockMs) / 1000);
  return `Make markets on the ${v.base}-USDT perpetual swap on ${v.label}. Each block here is a ${tick} tick; \`horizonBlocks\` (~${horizonS} s) is the horizon. Every tick one post-only limit order goes on the side you pick, just inside the touch, replacing the previous one. It never crosses, so we earn \`spreadBps\` rather than pay it, less a maker fee of about ${feeBps} bps per fill. A bid fill makes our position longer and an ask fill makes it shorter; being short is as easy as being long. The cost is adverse selection: a taker fills us exactly when the market is about to run the other way. Pick the side whose inventory you want to be holding \`horizonBlocks\` from now.`;
}
...
export const createModel = (venue: VenueInfo): Model => (config.model === "jev" ? new JevModel(venue) : new MockModel());
