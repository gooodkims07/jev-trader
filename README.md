...
- Stops and take-profits (all off by default; any venue). `RISK_SESSION_STOP_LOSS` / `RISK_SESSION_TAKE_PROFIT`: when this run's P&L (fees and unrealized included) reaches -stop or +take in the quote currency, the bot closes its position and stops. `RISK_POSITION_STOP_PCT` / `RISK_POSITION_TAKE_PCT`: when the position has moved that many percent against or in favour of its average entry, the bot closes it and trades on. Closing is slow on purpose: each block a reduce-only post-only order at the touch for the whole position, no market orders, so a fast market can run past the level while it closes.
- `OKX_EMERGENCY_STOP_PCT` (default off): an exchange-side stop on OKX for the whole position (conditional, `closeFraction` 1, reduce-only, market on trigger, mark price) at that percent from the average entry. It works with the bot down, is re-placed when the position flips or its entry moves by more than 0.5% of price, OKX drops it when the position closes, and it is left in place when the bot stops with a position open.
- Jev's OKX question differs from Kuru's (which is unchanged): each tick it answers bid, ask, or **skip**, judging whether a fill on that side is likely to be worth more than the maker fee after `HORIZON_BLOCKS`. Skip posts nothing and takes our resting order off the book (`totals.skips`; the dashboard shows SKIP). Its state adds `lookbackBlocks`, our `position` (side, size, entry, unrealized bps, cap) and `costs` (maker fee bps, funding rate). `MIN_SIDE_PROB` (e.g. 0.3) overrules a skip when the likelier of bid and ask is at least that probable: that side is posted (`quote.overSkip`). `LOOKBACK_BLOCKS` sets how far back the inputs look (taker flow, sampled mids), separately from the horizon; it defaults to `HORIZON_BLOCKS`, as on Kuru.
